...
function cargarCancionesFavoritas() {
  ...
  xhttp.onreadystatechange = function() {
      if (this.readyState == 4 && this.status == 200) {
          ...
          // Añadir un breve retraso para asegurar que se aplique el espacio
          setTimeout(function() {
...
}, 200); // Añadir un retardo para asegurar que el DOM se ha actualizado
}
...
function aplicarEspacioReproductor() {
  console.log("Aplicando espacio para el reproductor");
  
  // Obtenemos todas las posibles tablas
  const tablaCanciones = document.getElementById('canciones-table');
  const tablaFavoritos = document.getElementById('tabla-favoritos') || 
                        document.querySelector('.favoritos-container table');
  
  // Verificamos si el reproductor está visible
  const reproductor = document.getElementById('reproductor-container');
  if (!reproductor || reproductor.style.display === 'none') {
      console.log("Reproductor no visible, no se requiere espacio");
      return; // No hacemos nada si el reproductor no está visible
  }
  
  // Añadir espaciadores a ambas tablas si existen
  [tablaCanciones, tablaFavoritos].forEach(tabla => {
      if (!tabla) return;
      
      const tbody = tabla.querySelector('tbody');
      if (!tbody) return;
      
      // Eliminar espaciadores existentes para evitar duplicados
      const espaciadoresExistentes = tbody.querySelectorAll('[id^="reproductor-espaciador"]');
      espaciadoresExistentes.forEach(e => e.remove());
      
      // Crear y añadir nuevos espaciadores
      const espaciador = document.createElement('tr');
      espaciador.id = 'reproductor-espaciador-principal';
      espaciador.innerHTML = '<td colspan="6" style="height: 175px;"></td>';
      tbody.appendChild(espaciador);
      
      console.log("Espacio añadido a tabla", tabla.id || "sin ID");
  });
}